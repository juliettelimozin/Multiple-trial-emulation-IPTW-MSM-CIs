...
iters <- 1000
coefs_PP_noint <- matrix(,10,iters)
...
for (i in 1:iters){
  ...
  tryCatch({
    ...
    coefs_PP_noint[,i] <- pull(predicted_probas_PP_sample, survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
save(coefs_PP_noint, file = paste("coefs_PP_noint_",as.character(l),".rda", sep = ""))
