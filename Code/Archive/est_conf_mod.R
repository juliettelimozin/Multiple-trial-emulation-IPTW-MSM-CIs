...
iters <- 1000
coefs_PP_mod <- matrix(,nrow = 10, ncol = iters)
...
for (i in 1:iters){
  ...
  tryCatch({
    ...
    coefs_PP_mod[,i] <- pull(predicted_probas_PP_sample, survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
save(coefs_PP_mod, file = paste("coefs_PP_mod_",as.character(l),".rda", sep = ""))
