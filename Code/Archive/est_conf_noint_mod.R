...
iters <- 1000
coefs_PP_noint_mod <- matrix(,10,iters)
...
for (i in 1:iters){
  ...
  tryCatch({
    ...
    simdata_censored_conf<-DATA_GEN_censored_modified(1000, 10, conf = j)
    ...
    coefs_PP_noint_mod[,i] <- pull(predicted_probas_PP_sample, survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
save(coefs_PP_noint_mod, file = paste("coefs_PP_noint_mod_",as.character(l),".rda", sep = ""))
