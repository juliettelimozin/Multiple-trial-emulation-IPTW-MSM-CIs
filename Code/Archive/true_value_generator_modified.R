...
conf <- 1:9/10
true_value_conf_modified <- array(,dim = c(10,2,9))
true_value_treat_modified <- array(,dim = c(10,2,9))

for (i in conf){
  simdata_censored_conf_treat<-DATA_GEN_censored_modified(1000000, 10, conf = i, all_treat = T, censor = F)
  simdata_censored_conf_control<-DATA_GEN_censored_modified(1000000, 10, conf = i, all_control = T, censor = F)
  ...
  simdata_censored_treat_treat<-DATA_GEN_censored_modified(1000000, 10, treat_prev = i, all_treat = T, censor = F)
  simdata_censored_treat_control<-DATA_GEN_censored_modified(1000000, 10, treat_prev = i, all_control = T, censor = F)
