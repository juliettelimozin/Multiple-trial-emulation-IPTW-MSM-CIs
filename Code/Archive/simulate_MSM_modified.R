...
DATA_GEN_censored_modified<-function(ns, nv, conf = 0.5, treat_prev = 0, all_treat = FALSE, all_control = FALSE, censor = TRUE){   # ns= number of subjects, nv=no of visits including baseline visit
  ...
  for (k in 2:nvisit){  
    ...
    lpp<- as.numeric(treat_prev) + Ap[seqlist[[k]]]-X1[seqlist[[k]]]+as.numeric(conf)*X2[seqlist[[k]]]-X3[seqlist[[k]]]+1.2*X4[seqlist[[k]]]-1.5*(age[seqlist[[k]]]-35)/12
    P0[[k]]<-1/(1+exp(-lpp))
    ...
    lp<- -5 -1.2*A[seqlist[[k]]]+0.5*X1[seqlist[[k]]]+as.numeric(conf)*X2[seqlist[[k]]]+X3[seqlist[[k]]]+X4[seqlist[[k]]]+0.5*(age[seqlist[[k]]]-35)/12
