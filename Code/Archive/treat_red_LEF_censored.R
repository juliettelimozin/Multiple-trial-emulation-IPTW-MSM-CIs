...
CI_bootstrap_treat_PP_red <- array(,dim = c(5,2,iters))
CI_sandwich_treat_PP_red <- array(, dim = c(5,2,iters))
CI_LEF_outcome_treat_PP_red <- array(, dim = c(5,2,iters))
CI_LEF_both_treat_PP_red <- array(, dim = c(5,2,iters))

computation_time_treat <- array(,dim = c(4,iters))

est_treat <- array(, dim = c(5,iters))
...
for (i in 1:iters){
  ...
  tryCatch({
    simdata_censored_treat<-DATA_GEN_censored_reduced(1000, 5, treat_prev = j)
    ...
    computation_time_treat[1,i] <- (proc.time() - time)[[3]]
    CI_bootstrap_treat_PP_red[,1,i] <- surv_PP_difference_boostrap_estimates_treat$lb
    CI_bootstrap_treat_PP_red[,2,i] <- surv_PP_difference_boostrap_estimates_treat$ub
    ...
    surv_PP_difference_LEF_outcome_estimates_treat$ub <- apply(surv_PP_difference_LEF_outcome_estimates_treat,
                                                              ...
                                                              quantile,
                                                              probs = c(0.975))
    computation_time_treat[2,i] <- (proc.time() - time)[[3]]
    CI_LEF_outcome_treat_PP_red[,1,i] <- surv_PP_difference_LEF_outcome_estimates_treat$lb
    CI_LEF_outcome_treat_PP_red[,2,i] <- surv_PP_difference_LEF_outcome_estimates_treat$ub
    time <- proc.time()
    ...
    computation_time_treat[3,i] <- (proc.time() - time)[[3]]
    CI_LEF_both_treat_PP_red[,1,i] <- surv_PP_difference_LEF_both_estimates_treat$lb
    CI_LEF_both_treat_PP_red[,2,i] <- surv_PP_difference_LEF_both_estimates_treat$ub
    ...
    computation_time_treat[4,i] <- (proc.time() - time)[[3]]
    CI_sandwich_treat_PP_red[,1,i] <- surv_PP_difference_sandwich_estimates$lb
    CI_sandwich_treat_PP_red[,2,i] <- surv_PP_difference_sandwich_estimates$ub
    ...
    est_treat[,i] <- pull(predicted_probas_PP,survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
print(paste0("% not pos def: ", not_pos_def*100/iters))
save(CI_bootstrap_treat_PP_red, file = paste("CI_bootstrap_treat_PP_red_low_",as.character(l),".rda", sep = ""))
save(CI_sandwich_treat_PP_red, file = paste("CI_sandwich_treat_PP_red_low_",as.character(l),".rda", sep = ""))
save(CI_LEF_outcome_treat_PP_red, file = paste("CI_LEF_outcome_treat_PP_red_low_",as.character(l),".rda", sep = ""))
save(CI_LEF_both_treat_PP_red, file = paste("CI_LEF_both_treat_PP_red_low_",as.character(l),".rda", sep = ""))
save(computation_time_treat, file = paste("computation_time_treat_low_",as.character(l),".rda", sep = ""))
save(est_treat, file = paste("est_treat_red_low_", as.character(l), ".rda", sep = ""))
