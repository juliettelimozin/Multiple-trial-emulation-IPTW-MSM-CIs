...
surv_PP_ratio_boostrap_estimates <-as.data.frame(matrix(,5,bootstrap_iter))
ptm <- proc.time()
...
ggplot(data = predicted_probas_PP, aes(followup_time)) +
  ...
  geom_stepribbon(aes(ymin = survival_difference_lb_LEF, 
                      ymax = survival_difference_ub_LEF, color = "LEF variance weight func"), alpha = 0.1) +
  #geom_stepribbon(aes(ymin = survival_difference_lb, 
  #                    ymax = survival_difference_ub, color = "Direct boot weight func"), alpha = 0.3) +
  #scale_color_manual(name = "CI method", values = c("LEF variance weight func"= "red", "Direct boot weight func" = "blue")) +
