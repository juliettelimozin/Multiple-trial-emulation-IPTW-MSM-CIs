...
CI_bootstrap_treat_PP <- array(,dim = c(10,2,iters))
CI_sandwich_treat_PP <- array(,dim = c(10,2,iters))
...
for (i in 1:iters){
  ...
  surv_PP_difference_boostrap_estimates_treat <-as.data.frame(matrix(,10,bootstrap_iter))
  
  for (k in 1:bootstrap_iter){
    boot_data_treat <- bootstrap_construct(simdata_censored_treat)
    write.csv(boot_data_treat, paste(data_direction,"/boot_data_treat_PP.csv",sep = ""),row.names = F)
    
    ############################################################################
    PP_boot_treat <- RandomisedTrialsEmulation::initiators(paste(data_direction,"/boot_data_treat_PP.csv",sep = ""), id='ID', period='t', treatment='A', outcome='Y', eligible ='eligible', cense = 'C',
                                                           ...
                                                           use_weight=1, use_censor=1, case_control = 0, data_dir =data_direction, numCores = 4, quiet = TRUE)
    switch_data_boot <- read.csv(paste(data_direction,"/switch_data.csv",sep = ""))
    ...
    surv_PP_difference_boostrap_estimates_treat[,k] <- predicted_probas_PP_boot[,2] - predicted_probas_PP_boot[,3]
  ...
  CI_bootstrap_treat_PP[,1,i] <- surv_PP_difference_boostrap_estimates_treat$lb
  CI_bootstrap_treat_PP[,2,i] <- surv_PP_difference_boostrap_estimates_treat$ub
  
  ############################SANDWICH #######################################
  
  write.csv(simdata_censored_treat, file= paste(data_direction,"/MSM_censor_treat_PP.csv",sep = ""), row.names = F) 
  
  
  PP <-RandomisedTrialsEmulation::initiators(paste(data_direction,"/MSM_censor_treat_PP.csv",sep = ""), id='ID', period='t', treatment='A', outcome='Y', eligible ='eligible', cense = 'C',
                                             model_switchd =c( 'X1', 'X2', 'X3', 'X4', 'age_s'),
                                             cov_switchd = c( 'X1', 'X2', 'X3', 'X4', 'age_s'),
                                             outcomeCov_var=c( 'X3', 'X4', 'age_s'), outcomeCov =c('X3', 'X4', 'age_s'), model_var = c('assigned_treatment'),
                                             cov_censed = c( 'X1', 'X2','X3', 'X4', 'age_s'), model_censed =c( 'X1', 'X2','X3', 'X4', 'age_s'), pool_cense=1,
  ...
  switch_data <- read.csv(paste(data_direction,"/switch_data.csv",sep = ""))
  ...
  CI_sandwich_treat_PP[,1,i] <- surv_PP_difference_sandwich_estimates$lb
  CI_sandwich_treat_PP[,2,i] <- surv_PP_difference_sandwich_estimates$ub
