...
iters <- 1000
treat_PP_noint <- matrix(,10,iters)
...
for (i in 1:iters){
  ...
  tryCatch({
    ...
    treat_PP_noint[,i] <- pull(predicted_probas_PP_sample, survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
save(treat_PP_noint, file = paste("treat_PP_noint_",as.character(l),".rda", sep = ""))
