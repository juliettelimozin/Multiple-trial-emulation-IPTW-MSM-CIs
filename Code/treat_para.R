...
for (i in 1:iters){
  ...
  tryCatch({
    ...
    for (k in 1:bootstrap_iter) {
      boot_data_treat[[k]] <- bootstrap_construct(simdata_censored_treat)
    }
    
    surv_PP_difference_boostrap_estimates_treat <- foreach(k = 1:bootstrap_iter, .combine=cbind, .export=c("boot_data_treat")) %dopar% {
