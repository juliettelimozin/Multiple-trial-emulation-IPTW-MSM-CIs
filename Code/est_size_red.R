...
iters <- 1000
size_PP_red <- matrix(,nrow = 5, ncol = iters)
...
for (i in 1:iters){
  ...
  tryCatch({
    ...
    PP_prep <- RandomisedTrialsEmulation::data_preparation(simdata_censored_size, id='ID', period='t', treatment='A', outcome='Y', 
    ...
    size_PP_red[,i] <- pull(predicted_probas_PP_sample, survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
save(size_PP_red, file = paste("size_PP_red_",as.character(l),".rda", sep = ""))
