...
CI_bootstrap_coefs_PP_red_big <- array(,dim = c(5,2,iters))
CI_sandwich_coefs_PP_red_big <- array(, dim = c(5,2,iters))
CI_LEF_outcome_coefs_PP_red_big <- array(, dim = c(5,2,iters))
CI_LEF_both_coefs_PP_red_big <- array(, dim = c(5,2,iters))

est_conf_big <- array(, dim = c(5,iters))

computation_time_coefs_big <- array(,dim = c(4,iters))

l <- as.numeric(Sys.getenv('SLURM_ARRAY_TASK_ID'))
j <- as.numeric(l/10)
not_pos_def <- 0.0

data_direction <- paste("~/rds/hpc-work/models_conf_",l,sep = "")
...
for (i in 1:iters){
  ...
  tryCatch({
    simdata_censored_conf<-DATA_GEN_censored_reduced(5000, 5, conf = j)
    PP_prep <- TrialEmulation::data_preparation(simdata_censored_conf, id='ID', period='t', treatment='A', outcome='Y', 
    ...
    time <- proc.time()
    ############################# DIRECT BOOTSTRAP #############################
    surv_PP_difference_boostrap_estimates_conf <-as.data.frame(matrix(,5,bootstrap_iter))
    surv_PP_difference_boostrap_estimates_conf <- foreach(k = 1:bootstrap_iter, .combine=cbind) %dopar% {
    ...
    computation_time_coefs_big[1,i] <- (proc.time() - time)[[3]]
    CI_bootstrap_coefs_PP_red_big[,1,i] <- surv_PP_difference_boostrap_estimates_conf$lb
    CI_bootstrap_coefs_PP_red_big[,2,i] <- surv_PP_difference_boostrap_estimates_conf$ub
    ...
    surv_PP_difference_LEF_outcome_estimates_conf <- as.data.frame(matrix(,5,bootstrap_iter))
    surv_PP_difference_LEF_outcome_estimates_conf <- foreach(k = 1:bootstrap_iter, .combine=cbind) %dopar% {
    ...
    computation_time_coefs_big[2,i] <- (proc.time() - time)[[3]]
    CI_LEF_outcome_coefs_PP_red_big[,1,i] <- surv_PP_difference_LEF_outcome_estimates_conf$lb
    CI_LEF_outcome_coefs_PP_red_big[,2,i] <- surv_PP_difference_LEF_outcome_estimates_conf$ub
    ...
    surv_PP_difference_LEF_both_estimates_conf <- as.data.frame(matrix(,5,bootstrap_iter))
    surv_PP_difference_LEF_both_estimates_conf <- foreach(k = 1:bootstrap_iter, .combine=cbind) %dopar% {
      weights_table_boot <- data.frame(id = 1:5000) %>% 
        rowwise() %>% 
        dplyr::mutate(weight_boot = length(boot_data_conf[[k]][boot_data_conf[[k]] == id])) #bootstrap weight is number of times they were sampled
    ...
    computation_time_coefs_big[3,i] <- (proc.time() - time)[[3]]
    CI_LEF_both_coefs_PP_red_big[,1,i] <- surv_PP_difference_LEF_both_estimates_conf$lb
    CI_LEF_both_coefs_PP_red_big[,2,i] <- surv_PP_difference_LEF_both_estimates_conf$ub
    ...
    computation_time_coefs_big[4,i] <- (proc.time() - time)[[3]]
    CI_sandwich_coefs_PP_red_big[,1,i] <- surv_PP_difference_sandwich_estimates$lb
    CI_sandwich_coefs_PP_red_big[,2,i] <- surv_PP_difference_sandwich_estimates$ub
    ...
    est_conf_big[,i] <- pull(predicted_probas_PP,survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
print(paste0("% not pos def: ", not_pos_def*100/iters))
save(CI_bootstrap_coefs_PP_red_big, file = paste("CI_bootstrap_coefs_PP_red_big_low_",as.character(l),".rda", sep = ""))
save(CI_sandwich_coefs_PP_red_big, file = paste("CI_sandwich_coefs_PP_red_big_low_",as.character(l),".rda", sep = ""))
save(CI_LEF_outcome_coefs_PP_red_big, file = paste("CI_LEF_outcome_coefs_PP_red_big_low_",as.character(l),".rda", sep = ""))
save(CI_LEF_both_coefs_PP_red_big, file = paste("CI_LEF_both_coefs_PP_red_big_low_",as.character(l),".rda", sep = ""))
save(computation_time_coefs_big, file = paste("computation_time_coefs_big_low_",as.character(l),".rda", sep = ""))
save(est_conf_big, file = paste("est_conf_red_big_low_", as.character(l), ".rda", sep = ""))
