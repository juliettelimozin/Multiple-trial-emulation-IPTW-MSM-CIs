...
iters <- 1000
treat_PP_noint_mod <- matrix(,10,iters)
...
for (i in 1:iters){
  ...
  tryCatch({
    ...
    simdata_censored_treat<-DATA_GEN_censored_modified(1000, 10, treat_prev = j)
    ...
    treat_PP_noint_mod[,i] <- pull(predicted_probas_PP_sample, survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
save(treat_PP_noint_mod, file = paste("treat_PP_noint_mod_",as.character(l),".rda", sep = ""))
