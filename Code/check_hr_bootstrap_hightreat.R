...
hr_estimates_boot <- array(, dim = c(bootstrap_iter))
hr_estimates_sandwich <- array(, dim = c(sampling_size))
mrd_estimates_boot <- array(, dim = c(5,bootstrap_iter))
mrd_estimates_sandwich <- array(, dim = c(5,sampling_size))
mrd_estimates_LEF_outcome <- array(, dim = c(5,bootstrap_iter))
mrd_estimates_LEF_both <- array(, dim = c(5,bootstrap_iter))

if (l %in% 1:9){
  mrd_estimates_jackknife_mvn <- array(, dim = c(5,sampling_size))
  mrd_estimates_jackknife_wald <- array(, dim = c(5,as.numeric(scenarios[l,1])))
}
...
for (k in 1:bootstrap_iter){
  ...
  hr_estimates_boot[k] <- PP_boot$model$coefficients['assigned_treatment']
  ...
  mrd_estimates_boot[,k]<- pull(predicted_probas_PP_boot,mrd)
}
...
for (k in 1:bootstrap_iter){
  ...
  predicted_probas_PP_boot <- fitting_data_treatment %>% 
    ...
    dplyr::summarise(survival_treatment = mean(cum_hazard_treatment),
                     survival_control = mean(cum_hazard_control),
                     mrd = survival_control - survival_treatment)
  mrd_estimates_LEF_outcome[,k] <- pull(predicted_probas_PP_boot, mrd)
...
for(k in 1:bootstrap_iter){
  ...
  mrd_estimates_LEF_both[,k] <- pull(predicted_probas_PP_boot, mrd)
}
...
if (l %in% 1:9){
  ...
  for (k in 1:lenid){
    ...
    predicted_probas_PP_boot <- fitting_data_treatment_boot %>% 
      ...
      dplyr::summarise(survival_treatment = mean(cum_hazard_treatment),
                       survival_control = mean(cum_hazard_control),
                       mrd = survival_control-survival_treatment)
    mrd_estimates_jackknife_wald[,k] <- pull(predicted_probas_PP_boot,mrd)
  }
  ...
  jackknife_theta_var <- ((as.numeric(scenarios[l,1])-1)/as.numeric(scenarios[l,1]))*jackknife_theta_var
  sampling_size <- 200
  ...
  for(k in 1:sampling_size){
    ...
    mrd_estimates_jackknife_mvn[,k] <- pull(predicted_probas_PP_sample, mrd)
  }
  
  jackknife_mrd_se <- array(,dim = c(5))
  for (t in 1:5){
    jackknife_mrd_se[t] <- sqrt(((as.numeric(scenarios[l,1])-1)/as.numeric(scenarios[l,1]))*sum((mrd_estimates_jackknife_wald[t,] - mean(mrd_estimates_jackknife_wald[t,]))^2))
  }

}
############### SANDWICH #########################
covariance_mat <-PP$robust$matrix
if (all(eigen(covariance_mat)$values > 0) == T){
  ...
  for (k in 1:sampling_size){
    ...
    mrd_estimates_sandwich[,k] <- pull(predicted_probas_PP_sample,mrd)
  }
}

save(hr_estimates_boot, file = paste("hr_estimates_boot_low_",as.character(l),".rda", sep = ""))
save(hr_estimates_sandwich, file = paste("hr_estimates_sandwich_low_",as.character(l),".rda", sep = ""))
save(mrd_estimates_boot, file = paste("mrd_estimates_boot_low_",as.character(l),".rda", sep = ""))
save(mrd_estimates_sandwich, file = paste("mrd_estimates_sandwich_low_",as.character(l),".rda", sep = ""))
save(mrd_estimates_LEF_outcome, file = paste("mrd_estimates_LEF_outcome_low_",as.character(l),".rda", sep = ""))
save(mrd_estimates_LEF_both, file = paste("mrd_estimates_LEF_both_low_",as.character(l),".rda", sep = ""))

if(l %in% 1:9){
  save(mrd_estimates_jackknife_wald, file = paste("mrd_estimates_jackknife_wald_low_",as.character(l),".rda", sep = ""))
  save(mrd_estimates_jackknife_mvn, file = paste("mrd_estimates_jackknife_mvn_low_",as.character(l),".rda", sep = ""))
  save(jackknife_mrd_se, file = paste("jackknife_mrd_se_low_",as.character(l),".rda", sep = ""))
}
