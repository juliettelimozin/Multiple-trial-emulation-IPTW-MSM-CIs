...
library(tidyverse)
library(tidyr)
setwd("/home/juliette/UniHPC")
source("/home/juliette/Multiple-trial-emulation-IPTW-MSM-CIs/Code/simulate_MSM_simplified.R")
source("/home/juliette/Multiple-trial-emulation-IPTW-MSM-CIs/Code/weight_func_efficient.R")

set.seed(20250228)
seeds <- floor(runif(1000)*10^8)

library(TrialEmulation, lib.loc = '/home/juliette/R/x86_64-redhat-linux-gnu-library/4.3')
library(MASS)
library(sandwich)
library(doParallel)
library(doRNG)
...
data_direction <- paste("/home/juliette/UniHPC/models_scenario_low_",l,sep = "")
...
source("/home/juliette/Multiple-trial-emulation-IPTW-MSM-CIs/Code/weight_func.R")

computation_time_old <- array(,dim = c(6,iters))
CI_bootstrap_PP_red_old  <- array(,dim = c(5,2,iters))
CI_sandwich_PP_red_old  <- array(, dim = c(5,2,iters))
CI_LEF_outcome_PP_red_old  <- array(, dim = c(5,2,iters))
CI_LEF_both_PP_red_old  <- array(, dim = c(5,2,iters))
CI_jackknife_mvn_PP_red_old  <- array(, dim = c(5,2,iters))
CI_jackknife_wald_PP_red_old  <- array(, dim = c(5,2,iters))
...
for (i in 1:iters){
  tryCatch({
    ...
    computation_time_old[1,i] <- (proc.time() - time)[[3]]
    bootstrap_nas[i] <- sum(surv_PP_difference_boostrap_estimates[1,])
    CI_bootstrap_PP_red_old[,1,i] <- surv_PP_difference_boostrap_estimates[2:6,501]
    CI_bootstrap_PP_red_old[,2,i] <- surv_PP_difference_boostrap_estimates[2:6,502]
    ...
    computation_time_old[2,i] <- (proc.time() - time)[[3]]
    CI_LEF_outcome_PP_red_old[,1,i] <- surv_PP_difference_LEF_outcome_estimates$lb
    CI_LEF_outcome_PP_red_old[,2,i] <- surv_PP_difference_LEF_outcome_estimates$ub
    print('LEF both')
    ...
    computation_time_old[3,i] <- (proc.time() - time)[[3]]
    CI_LEF_both_PP_red_old[,1,i] <- surv_PP_difference_LEF_both_estimates$lb
    CI_LEF_both_PP_red_old[,2,i] <- surv_PP_difference_LEF_both_estimates$ub
    ...
    if(as.numeric(scenarios[l,1]) == 200){
      ...
      jackknife_SEs[,i]<- jacknife_mrd_se
      CI_jackknife_wald_PP_red_old[,,i] <- cbind(mrd - 1.96*jacknife_mrd_se, mrd + 1.96*jacknife_mrd_se)
      
      computation_time_old[4,i] <- (proc.time() - time)[[3]]
      jackknife_wald_mrd[,,i]<- t(list.rbind(jackknife_est_mrd[3,]))
      ...
      if(all(jackknife_est_mrd[1,] == 0)){
        ...
        CI_jackknife_mvn_PP_red_old[,,i] <- cbind(apply(surv_PP_difference_jackknife_estimates,
                                                    1,
                                                    quantile,
                                                    probs = c(0.025)), 
                                              apply(surv_PP_difference_jackknife_estimates,
        ...
        computation_time_old[5,i] <- (proc.time() - time)[[3]]
        jackknife_mvn_mrd[,,i] <- as.matrix(surv_PP_difference_jackknife_estimates)
      } else {
        jackknife_nas[i] <- sum(list.rbind(jackknife_est_mrd[1,]))
      }
    ...
    computation_time_old[6,i] <- (proc.time() - time)[[3]]
    CI_sandwich_PP_red_old[,1,i] <- surv_PP_difference_sandwich_estimates$lb
    CI_sandwich_PP_red_old[,2,i] <- surv_PP_difference_sandwich_estimates$ub
    
    print('All done')
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
print(rowMeans(computation_time, na.rm = TRUE))
print(rowMeans(computation_time_old, na.rm = TRUE))
