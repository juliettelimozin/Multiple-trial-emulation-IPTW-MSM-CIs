...
CI_bootstrap_size_PP_red <- array(,dim = c(5,2,iters))
CI_sandwich_size_PP_red <- array(, dim = c(5,2,iters))
CI_LEF_outcome_size_PP_red <- array(, dim = c(5,2,iters))
CI_LEF_both_size_PP_red <- array(, dim = c(5,2,iters))

computation_time_size <- array(,dim = c(4,iters))

est_size <- array(, dim = c(5,iters))

l <- as.numeric(Sys.getenv('SLURM_ARRAY_TASK_ID'))
j <-as.numeric(100*l)
not_pos_def <- 0.0

data_direction <- paste("~/rds/hpc-work/models_size_",l,sep = "")
...
for (i in 1:iters){
  ##################### SAMPLE SIZE ########################################
  tryCatch({
    simdata_censored_size<-DATA_GEN_censored_reduced(j, 5)
    PP_prep <- TrialEmulation::data_preparation(simdata_censored_size, id='ID', period='t', treatment='A', outcome='Y', 
    ...
    for (k in 1:bootstrap_iter) {
      boot_data_size[[k]] <- sort(sample(unique(switch_data$id), length(unique(switch_data$id)), replace = TRUE))
    }
    time <- proc.time()
    ############################# DIRECT BOOTSTRAP #############################
    surv_PP_difference_boostrap_estimates_size <-as.data.frame(matrix(,5,bootstrap_iter))
    surv_PP_difference_boostrap_estimates_size <- foreach(k = 1:bootstrap_iter, .combine=cbind) %dopar% {
      
      weights_table_boot <- data.frame(id = 1:j) %>% 
        rowwise() %>% 
        dplyr::mutate(weight_boot = length(boot_data_size[[k]][boot_data_size[[k]] == id])) #bootstrap weight is number of times they were sampled
      
      IP_model <- weight_func_bootstrap(data = simdata_censored_size, expanded_data = switch_data, switch_d_cov = ~ X2 + X4, cense = 'C', cense_d_cov = ~ X2 + X4,
                                        ...
                                        boot_idx = boot_data_size[[k]], remodel = TRUE, quiet = TRUE)
    ...
    computation_time_size[1,i] <- (proc.time() - time)[[3]]
    CI_bootstrap_size_PP_red[,1,i] <- surv_PP_difference_boostrap_estimates_size$lb
    CI_bootstrap_size_PP_red[,2,i] <- surv_PP_difference_boostrap_estimates_size$ub
    ...
    surv_PP_difference_LEF_outcome_estimates_size <- as.data.frame(matrix(,5,bootstrap_iter))
    surv_PP_difference_LEF_outcome_estimates_size <- foreach(k = 1:bootstrap_iter, .combine=cbind) %dopar% {
    ...
    surv_PP_difference_LEF_outcome_estimates_size$lb <-apply(surv_PP_difference_LEF_outcome_estimates_size,
                                                              1,
                                                              quantile,
                                                              probs = c(0.025))
    surv_PP_difference_LEF_outcome_estimates_size$ub <- apply(surv_PP_difference_LEF_outcome_estimates_size,
                                                               1,
                                                               quantile,
                                                               probs = c(0.975))
    
    
    computation_time_size[2,i] <- (proc.time() - time)[[3]]
    CI_LEF_outcome_size_PP_red[,1,i] <- surv_PP_difference_LEF_outcome_estimates_size$lb
    CI_LEF_outcome_size_PP_red[,2,i] <- surv_PP_difference_LEF_outcome_estimates_size$ub
    ...
    surv_PP_difference_LEF_both_estimates_size <- as.data.frame(matrix(,5,bootstrap_iter))
    surv_PP_difference_LEF_both_estimates_size <- foreach(k = 1:bootstrap_iter, .combine=cbind) %dopar% {
      weights_table_boot <- data.frame(id = 1:j) %>% 
        rowwise() %>% 
        dplyr::mutate(weight_boot = length(boot_data_size[[k]][boot_data_size[[k]] == id])) #bootstrap weight is number of times they were sampled
      ...
      IP_model <- weight_func_bootstrap(data = simdata_censored_size, expanded_data = switch_data, switch_d_cov = ~ X2 + X4, cense = 'C', cense_d_cov = ~ X2 + X4,
                                        ...
                                        boot_idx = boot_data_size[[k]], remodel = FALSE, quiet = TRUE)
    ...
    surv_PP_difference_LEF_both_estimates_size$lb <-apply(surv_PP_difference_LEF_both_estimates_size,
                                                           1,
                                                           quantile,
                                                           probs = c(0.025))
    surv_PP_difference_LEF_both_estimates_size$ub <- apply(surv_PP_difference_LEF_both_estimates_size,
                                                            1,
                                                            quantile,
                                                            probs = c(0.975))
    ...
    computation_time_size[3,i] <- (proc.time() - time)[[3]]
    CI_LEF_both_size_PP_red[,1,i] <- surv_PP_difference_LEF_both_estimates_size$lb
    CI_LEF_both_size_PP_red[,2,i] <- surv_PP_difference_LEF_both_estimates_size$ub
    ...
    computation_time_size[4,i] <- (proc.time() - time)[[3]]
    CI_sandwich_size_PP_red[,1,i] <- surv_PP_difference_sandwich_estimates$lb
    CI_sandwich_size_PP_red[,2,i] <- surv_PP_difference_sandwich_estimates$ub
    ...
    est_size[,i] <- pull(predicted_probas_PP,survival_difference)
    
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
print(paste0("% not pos def: ", not_pos_def*100/iters))
save(CI_bootstrap_size_PP_red, file = paste("CI_bootstrap_size_PP_red_low_",as.character(l),".rda", sep = ""))
save(CI_sandwich_size_PP_red, file = paste("CI_sandwich_size_PP_red_low_",as.character(l),".rda", sep = ""))
save(CI_LEF_outcome_size_PP_red, file = paste("CI_LEF_outcome_size_PP_red_low_",as.character(l),".rda", sep = ""))
save(CI_LEF_both_size_PP_red, file = paste("CI_LEF_both_size_PP_red_low_",as.character(l),".rda", sep = ""))
save(computation_time_size, file = paste("computation_time_size_low_",as.character(l),".rda", sep = ""))
save(est_size, file = paste("est_size_red_low_", as.character(l), ".rda", sep = ""))
